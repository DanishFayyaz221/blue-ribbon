...
/** `navy-pill` is the filled twin of `outline-pill` — the phone property
 *  page draws Share solid next to an outlined Enquire, per the mobile comp. */
type Variant = "outline" | "outline-pill" | "navy-pill";
...
export function ShareTrigger({
  ...
  const base =
    variant === "navy-pill"
      ? "group relative isolate flex h-[48px] items-center justify-center overflow-hidden rounded-[24px] border border-brand-navy bg-brand-navy font-display text-[14px] font-semibold text-white transition-colors duration-300 before:absolute before:-inset-px before:z-0 before:translate-y-full before:bg-brand-navy-deep before:transition-transform before:duration-400 before:ease-[cubic-bezier(0.65,0,0.35,1)] hover:before:translate-y-0"
