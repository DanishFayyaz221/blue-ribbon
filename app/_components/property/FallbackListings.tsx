...
export function FallbackListings({ items, relaxed, q, noun, clearHref }: Props) {
  ...
  return (
    <div>
      <div className="rounded-[16px] border border-brand-silver/60 bg-brand-soft/30 px-[20px] py-[20px] sm:px-[26px] sm:py-[24px]">
        <h3 className="font-display text-[clamp(15px,1.3vw,20px)] font-semibold text-brand-bunker">
          {title}
        </h3>
        <p className="mt-[8px] max-w-[560px] font-display text-[clamp(13px,0.95vw,15px)] leading-[1.6] text-brand-bunker/70">
          {message}
        </p>
        <Link
          href={clearHref}
          className="mt-[14px] inline-flex font-display text-[clamp(13px,0.95vw,14px)] font-semibold text-brand-navy underline underline-offset-4 hover:text-brand-navy-deep"
        >
          Clear filters
        </Link>
      </div>
