...
export function EnquiryModal({ open, onClose, agents = defaultAgents, listingId }: EnquiryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (status === "submitting" || !agree) return;
    setStatus("submitting");
    setErrorMsg(null);
    try {
      await api.submitPropertyEnquiry({
        firstName,
        lastName: lastName || undefined,
        email,
        mobile: mobile || undefined,
        message: message || undefined,
        helpTopic: help ?? undefined,
        listingId,
      });
      setStatus("success");
    } catch (err) {
    ...
    >
      ...
      <div className="relative z-10 flex w-full max-w-[760px] max-h-[calc(100vh-32px)] flex-col overflow-hidden rounded-[12px] bg-white shadow-[0_24px_60px_rgba(0,0,0,0.18)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-[clamp(16px,2vw,28px)] overflow-y-auto p-[clamp(20px,2.4vw,32px)]">
          ...
          {status === "success" ? (
            <div className="rounded-[10px] bg-[#F1F0ED] p-[24px] flex flex-col items-center justify-center text-center">
              <p className="font-display text-[18px] font-semibold text-brand-navy">Enquiry sent</p>
              ...
              <button
                type="button"
                onClick={handleClose}
                className="mt-6 h-[44px] rounded-[22px] bg-brand-navy px-[24px] font-display text-[13px] font-semibold text-white transition hover:bg-brand-navy-deep"
              >
                Close
              </button>
            </div>
          ...
          >
            ...
            {errorMsg && (
              <p className="mt-4 font-display text-[12px] text-red-600">{errorMsg}</p>
            )}
            <button
              type="submit"
