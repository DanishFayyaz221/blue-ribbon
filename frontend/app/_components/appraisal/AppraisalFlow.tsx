...
export function AppraisalFlow({
  ...
  // Searching moves on to the report — each path has its own page. Address is
  // carried over via query param so the details step can submit it.
  const handleSearch = () => {
    const base = isRental ? "/rental-report" : "/property-report";
    const target = address ? `${base}?a=${encodeURIComponent(address)}` : base;
    router.push(target);
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main>
        ...
        {step === "details" && (
          <DetailsStep
            reportType={isRental ? "rental" : "sales"}
...
function DetailsStep({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMsg(null);
    try {
      await api.submitAppraisal({
        name: `${first} ${last}`.trim() || first || "Website Enquirer",
        email,
      ...
      <div className="sm:hidden">
        ...
        <section className="container-page pb-[36px]">
          ...
          >
            ...
            />
            {errorMsg && (
              <p className="mt-[10px] text-center font-display text-[12px] text-red-600">{errorMsg}</p>
            )}
            <button
              type="submit"
              disabled={submitting}
              className="mt-[12px] h-[42px] w-full rounded-[22px] bg-brand-navy font-display text-[12.5px] font-semibold tracking-[0.05em] text-white transition hover:bg-brand-navy-deep disabled:opacity-60"
            >
        ...
        <div className="flex flex-col items-stretch gap-[22px] lg:flex-row lg:items-center lg:justify-center lg:gap-0">
          ...
          >
            ...
            <button
              type="submit"
              disabled={submitting}
              className="mt-[clamp(14px,1.25vw,22px)] h-[44px] w-full rounded-[22px] bg-brand-navy font-display text-[13px] font-semibold tracking-[0.05em] text-white transition hover:bg-brand-navy-deep disabled:opacity-60"
            >
              {submitting ? "SENDING..." : "NEXT"}
            </button>
          </form>
