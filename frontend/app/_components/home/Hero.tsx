...
function SearchBar() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (deal === "Sell") {
      router.push("/property-report-digital-appraisal");
      return;
    }
    const params = new URLSearchParams();
    if (query.trim()) params.set("suburb", query.trim());
    if (deal === "Rent") params.set("type", "Lease");
    const qs = params.toString();
    router.push(`/buy${qs ? `?${qs}` : ""}`);
  };

  return (
    <form onSubmit={handleSearch} className="mx-auto flex w-full max-w-[1280px] flex-col gap-[12px] sm:flex-row sm:items-stretch sm:gap-0">
...
function MobileSearch() {
  ...
  return (
    <div className="mx-auto w-full max-w-[420px]">
      <form onSubmit={handleSearch} className="flex h-[52px] w-full items-stretch overflow-hidden rounded-[12px] bg-white py-[6px] pl-[16px] pr-[6px]">
        <input
          type="text"
